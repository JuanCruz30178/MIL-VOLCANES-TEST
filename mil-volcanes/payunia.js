(function () {
  ...
  var originalCards = track ? Array.prototype.slice.call(track.querySelectorAll(".sight-card")) : [];
  ...
  function updateSightSlider() {
    ...
    sightCards.forEach(function (c, i) {
  ...
  function setupSightSlider() {
    ...
    for (var setIndex = 0; setIndex < 3; setIndex++) {
      originalCards.forEach(function (card, cardIndex) {
        ...
        clone.addEventListener("click", function () { selectSightCard(clone); });
        clone.addEventListener("keydown", function (e) {
          if (e.key === "Enter" || e.key === " ") { e.preventDefault(); selectSightCard(clone); }
        });
